...
def swap_cube(msg):
     abc=rasa_request("I am E1G1","E1G1")         
     txt_rasa=rasa_request(msg,"E1G1")
...
def put_cube_inside(msg):
     #abc=rasa_request("I am E2G1","E2G1")
     #txt_rasa=rasa_request(msg,"E2G1")
     #msg=communication(txt_rasa)
     txt_split="Error, I’m unable to put the item on shelf."
     txt_new=[]
     for i in range(0,len(txt_split),3):
          txt_new.append(' '.join(txt_split[i:i+3]))
     txt='\n'.join(txt_new)
     head_display_text(txt)
     msg=communication(txt_split)
     return msg
